...
export type MruCacheOptions<K, V> = PolicyCacheOptions<K, V>;

export class MruCache<K, V> extends EntryCountCache<K, V> {
  private readonly records = new Map<K, EntryRecord<V>>();

  constructor(options: MruCacheOptions<K, V> = {}) {
    super(options);
  ...
  private evictOne(): void {
    const key = lastKey(this.records);
    if (key === undefined) {
      return;
    }
