...
export type FifoCacheOptions<K, V> = PolicyCacheOptions<K, V>;

export class FifoCache<K, V> extends EntryCountCache<K, V> {
  private readonly records = new Map<K, EntryRecord<V>>();

  constructor(options: FifoCacheOptions<K, V> = {}) {
    super(options);
  ...
  private evictOne(): void {
    const key = firstKey(this.records);
    if (key === undefined) {
      return;
    }
