...
test("arrayContainsDuplicates() works", () => {
  ...
  expect(arrayContainsDuplicates([{ x: 1 }, { x: 1 }], deepEquals)).toStrictEqual(true);
  expect(arrayContainsDuplicates([{ x: 1 }, { x: 2 }], deepEquals)).toStrictEqual(false);
});

test("arrayBinarySearch() works", () => {
  expect(
    arrayBinarySearch(["a", "b", "c", "d", "e"], "c", (a: string, b: string) => a.localeCompare(b)),
  ).toStrictEqual(ok(2));
  expect(
    arrayBinarySearch(["a", "b", "c", "d", "e"], "z", (a: string, b: string) => a.localeCompare(b)),
  ).toStrictEqual(err(5));
  expect(
    arrayBinarySearch(["a", "b", "c", "d", "e"], "0", (a: string, b: string) => a.localeCompare(b)),
  ).toStrictEqual(err(0));
});
