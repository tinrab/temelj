...
test("request - cookie - encrypt", async () => {
  ...
  expect(!c.includes("hello")).toStrictEqual(true);
  expect(await decryptCookieValue(c, { password })).toStrictEqual("hello");
  expect(c.split("|").length).toStrictEqual(6);
  expect(c.split("|").every((s) => s.trim().length !== 0)).toStrictEqual(true);
